...
export async function POST(req) {
  try {
    ...
    const cronograma = await prisma.cronogramas.findUnique({
      where: { id: parseInt(cronograma_id) },
      select: {
        id: true,
        fecha_inicio: true,
      },
    ...
    if (!cronograma?.id) {
      throw new ApiError(400, 'Cronograma no encontrado');
    }
    ...
    const newRecord = await prisma.perforadores_forecast.create({ data });
